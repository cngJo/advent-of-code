...
function execute_movements_on_stacks(array &$stack, array $movements) {
    foreach ($movements as $movement) {
        ...
        // Get $amount of crates from the end of $stack[$from]
        $cratesToMove = array_slice($stack[$from], count($stack[$from]) - $amount, $amount);

        // Remove those creates from $stack[$from]
        $stack[$from] = array_slice($stack[$from], 0, count($stack[$from]) - $amount);

        // Add the moved crated to $stack[$to]
        $stack[$to] = array_merge($stack[$to], $cratesToMove);        
    }
}
